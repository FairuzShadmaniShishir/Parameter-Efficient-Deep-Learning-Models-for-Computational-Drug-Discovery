...
import numpy as np
train_path = '/home/f087s426/Research/Masters_Thesis/leash_BELKA/train.parquet'
test_path = '/home/f087s426/Research/Masters_Thesis/leash_BELKA/test.parquet'
...
samples_per_category = 400,000  # Adjust this number as needed, this is just an example for the tutorial.
...
brd4_df=brd4_df.sample(frac=1, random_state=42).reset_index(drop=True)
seh_df=seh_df.sample(frac=1, random_state=42).reset_index(drop=True)
hsa_df=hsa_df.sample(frac=1, random_state=42).reset_index(drop=True)


import pandas as pd
from sklearn.model_selection import train_test_split
brd4_train_df,brd4_test_df=train_test_split(brd4_df, test_size=0.2, random_state=42)
seh_train_df,seh_test_df=train_test_split(seh_df, test_size=0.2, random_state=42)
hsa_train_df,hsa_test_df=train_test_split(hsa_df, test_size=0.2, random_state=42)
...
def featurize_data_in_batches(smiles_list, labels_list, batch_size):
    ...
    for i in range(0, len(smiles_list), batch_size):
        ...
        batch_data_list = create_pytorch_geometric_graph_data_list_from_smiles_and_labels(smiles_batch, ids_batch,
                                                                                          labels_batch)
...
brd4_test_label = brd4_test_df['binds']
seh_test_label = seh_test_df['binds']
hsa_test_label = hsa_test_df['binds']
...
from sklearn.metrics import roc_curve, auc, matthews_corrcoef, classification_report, confusion_matrix,average_precision_score
import matplotlib.pyplot as plt

# `actual` = true labels (0 or 1)
# `final_predictions.binds` = predicted probabilities for positive class (float values between 0 and 1)
# `final_predictions.classes` = predicted class labels (0 or 1)

# ROC Curve and AUC
fpr, tpr, _ = roc_curve(seh_test_label, all_predictions[0].binds)
roc_auc = auc(fpr, tpr)
print("AUC:", roc_auc)
ap = average_precision_score(seh_test_label, all_predictions[0].binds)
print("AUCPR (Average Precision):", ap)
...
plt.title('SEH Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.grid(True)
plt.show()

# Matthews Correlation Coefficient
mcc = matthews_corrcoef(seh_test_label, (all_predictions[0].binds>=.92).astype(int))
print("Matthews Correlation Coefficient (MCC):", mcc)

# Classification Report
report = classification_report(seh_test_label, (all_predictions[0].binds>=.92).astype(int))
print("Classification Report:\n", report)

# Optional: Confusion Matrix
cm = confusion_matrix(seh_test_label, (all_predictions[0].binds>=.92).astype(int))
print("Confusion Matrix:\n", cm)


from sklearn.metrics import roc_curve, auc, matthews_corrcoef, classification_report, confusion_matrix
import matplotlib.pyplot as plt
...
# ROC Curve and AUC
fpr, tpr, _ = roc_curve(brd4_test_label, all_predictions[1].binds)
roc_auc = auc(fpr, tpr)
print("AUC:", roc_auc)
ap = average_precision_score(brd4_test_label, all_predictions[1].binds)
print("AUCPR (Average Precision):", ap)
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('BRD4 Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.grid(True)
plt.show()

# Matthews Correlation Coefficient
mcc = matthews_corrcoef(brd4_test_label, (all_predictions[1].binds>=.92).astype(int))
print("Matthews Correlation Coefficient (MCC):", mcc)

# Classification Report
report = classification_report(brd4_test_label, (all_predictions[1].binds>=.92).astype(int))
print("Classification Report:\n", report)

# Optional: Confusion Matrix
cm = confusion_matrix(brd4_test_label, (all_predictions[1].binds>=.92).astype(int))
print("Confusion Matrix:\n", cm)
...
# ROC Curve and AUC
fpr, tpr, _ = roc_curve(hsa_test_label, all_predictions[2].binds)
roc_auc = auc(fpr, tpr)
print("AUC:", roc_auc)
ap = average_precision_score(hsa_test_label, all_predictions[2].binds)
print("AUCPR (Average Precision):", ap)
# Plot ROC Curve
plt.figure()
plt.plot(fpr, tpr, color='darkorange', lw=2, label=f'ROC curve (AUC = {roc_auc:.2f})')
...
# Matthews Correlation Coefficient
mcc = matthews_corrcoef(hsa_test_label, (all_predictions[2].binds>=.92).astype(int))
print("HSA Matthews Correlation Coefficient (MCC):", mcc)

# Classification Report
report = classification_report(hsa_test_label, (all_predictions[2].binds>=.92).astype(int))
print("Classification Report:\n", report)

# Optional: Confusion Matrix
cm = confusion_matrix(hsa_test_label, (all_predictions[2].binds>=.92).astype(int))
print("Confusion Matrix:\n", cm)
